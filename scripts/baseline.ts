...
async function assertTestDatabase(
  ...
  if (!name.endsWith("_test")) {
    throw new Error(
      `Refusing destructive setup for ${name || "<unknown>"}; test database names must end in _test.`,
    );
  }
}
...
const CATALOG_QUERIES = {
  ...
  columns: `select table_schema as schema, table_name, column_name, data_type,
...
async function catalog(pool: Pool) {
  const entries = await Promise.all(
    Object.entries(CATALOG_QUERIES).map(async ([name, query]) => [
      name,
...
async function prepareBaselineDatabase(pool: Pool) {
  ...
  try {
    ...
    const baselineSql = (await readFile(BASELINE, "utf8")).replaceAll(
...
async function main() {
  ...
  try {
    ...
    if (
      process.env.CONFIRM_BASELINE_ADOPTION !== "I_HAVE_VERIFIED_SCHEMA_PARITY"
    ) {
      throw new Error(
        "Set CONFIRM_BASELINE_ADOPTION=I_HAVE_VERIFIED_SCHEMA_PARITY to record the baseline.",
      );
    }
    ...
    try {
      ...
      if (existing.rowCount)
        throw new Error(
          "Migration history is not empty; baseline adoption is only valid once.",
        );
      await client.query(
